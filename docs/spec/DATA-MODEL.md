...
| platform_name | text | NN | `'Sales League'` | `CK (length between 1 and 40)` — segunda linha da marca (a primeira é `company_name`); também compõe `document.title` |
| brand_preset | text | NN | `'esmeralda'` | `CK (in ('esmeralda','safira','ametista','ambar','coral','ciano','rosa'))` — o front aplica em `html[data-brand]`; as cores vivem em `styles/tokens.css`, com variante escurecida no tema claro (todas ≥ 4,5:1) |
| logo_data_url | text | — | — | `CK (null or (~ '^data:image/(png\|jpeg\|webp\|svg\+xml);base64,' and length <= 280000))` — logo do cliente embutida na própria linha (≈ 200 KB de imagem). Evita exigir bucket no Storage e um passo a mais na implantação; o navegador reduz a imagem antes de enviar. `null` no patch **apaga** (única chave de `update_app_settings` que não usa `coalesce`) |
| default_theme | text | NN | `'dark'` | `CK (in ('dark','light'))` — tema de quem ainda não escolheu o próprio; preferência salva no navegador sempre vence |
| updated_at | timestamptz | NN | now() | trigger |
| updated_by | uuid | — | — | FK profiles `ON DELETE SET NULL` |

Não contém segredo algum. RLS: SELECT `is_active_member()`; UPDATE `is_admin()`; sem INSERT/DELETE para `authenticated` (a linha vem do seed). Trigger de auditoria — `update_app_settings` troca `logo_data_url` por `'<logo>'` no payload auditado (base64 de 200 KB não vai para o `audit_log`).

**Marca antes do login.** A tela de login precisa da marca sem sessão, e `app_settings` é fechada para `anon`. Por isso existe `public.get_branding()` (§7.1): `security definer`, executável por `anon`, devolve **só** `company_name`, `platform_name`, `brand_preset`, `logo_data_url` e `default_theme`. Nenhuma outra coluna (metas, fuso, flags) sai por ela.
...
**`get_branding() returns jsonb`** — `security definer stable`; `grant execute to anon, authenticated`.
`jsonb_build_object` com `company_name`, `platform_name`, `brand_preset`, `logo_data_url`, `default_theme` de `app_settings` (§4.1). Existe porque a tela de login tem de mostrar a marca do cliente antes de haver sessão, e a tabela é fechada para `anon`. Não expõe nenhuma outra coluna. O front a consulta em `qk.branding()` e a invalida quando `update_app_settings` toca qualquer campo de marca.
...
**`update_app_settings(p_patch jsonb) returns app_settings`** — admin. Chaves: `company_name, xp_per_level, currency, timezone, target_*, rank_admins, auto_approve_members, platform_name, brand_preset, logo_data_url, default_theme`. As quatro últimas são a marca (§4.1): `logo_data_url` é a única chave que aceita `null` como valor real (apaga a logo — `case when p_patch ? 'logo_data_url' then ... else a.logo_data_url end`, não `coalesce`), e o payload auditado troca a data-URL por `'<logo>'`. `auto_approve_members` (boolean) só afeta cadastros futuros — quem já está `pending` continua pendente até o gestor aprovar (Apêndice B.21). `timezone` diferente do atual com `exists (select 1 from point_entries)` → RAISE `TIMEZONE_LOCKED` (detail "O fuso só pode ser alterado antes do primeiro lançamento."; o trigger `validate_timezone` repete a checagem para o `update` direto — Apêndice B.18). (Alternativa equivalente: `update` direto — a policy permite; a RPC existe para validar chaves e registrar auditoria. O front usa a RPC.)
...
| D.32 | MEDIUM | `base_points` em `admin_update_profile` não idempotente e falhava sem temporada | **aplicado** | Removido de §7.2/§4.6; nova RPC `record_initial_points` (§7.4) com `INITIAL_POINTS_EXISTS`, `source='system'` (não conta streak — sem string-matching no critério); teste (r). ⚠ FRONTEND-ARCH: tirar "pontos-base" do formulário e criar a ação separada — **atendido** (FRONTEND-ARCH §4.2, §4.5 features/team) |
...
| D.36 | MEDIUM | `activate_season` antes de `starts_at` deixava `active_season_id()` ≠ `season_for(now())`; `close_season` de temporada futura violava CK | **aplicado (B.20)** | §7.3 `SEASON_NOT_STARTED` em ambas; `close_season` devolve `warnings: ['gap_until_next_season']` + próxima temporada; §14.8. ⚠ FRONTEND-ARCH: "Ativar" desabilitado até a data; oferta de antecipar a próxima — **atendido** (FRONTEND-ARCH §4.2 `CloseSeasonPayload`, §4.5 features/settings) |
