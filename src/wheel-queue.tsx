...
import { Check, CirclePlus, Dices, Lock, Play, Search, ShieldCheck, Trash2, Unlock, UserPlus, Users, Zap } from "lucide-react";
...
function allowed(item: QueueEntry | ActiveTurn) { return Math.max(1, Number(item.attemptsAllowed || 1)); }
function used(item: QueueEntry | ActiveTurn) { return Math.max(0, Number(item.attemptsUsed || 0)); }
function normalizeQueue(items: QueueEntry[]) { return items.map(item => ({ ...item, attemptsAllowed: allowed(item), attemptsUsed: used(item) })); }

export function WheelQueue() {
  const [host, setHost] = useState<HTMLElement | null>(null);
  const [queue, setQueue] = useState<QueueEntry[]>(() => normalizeQueue(readList<QueueEntry>(QUEUE_KEY)));
  ...
  useEffect(() => {
    const sync = () => {
      ...
      if (!target) { target = document.createElement("div"); target.dataset.wheelQueue = "1"; main.append(target); }
      setHost(target);
    };
    const loadPeople = () => {
      setCustomPeople(readList<Person>(PEOPLE_KEY));
      setOverrides(readMap<Partial<Person>>(OVERRIDES_KEY));
      setQueue(normalizeQueue(readList<QueueEntry>(QUEUE_KEY)));
    ...
    const approve = (event: Event) => {
      ...
      let remaining = 0;
      setQueue(current => {
        const target = current.find(item => item.id === detail.queueId);
        if (!target) return current;
        const nextUsed = used(target) + 1;
        const max = allowed(target);
        remaining = Math.max(0, max - nextUsed);
        const next = remaining > 0
          ? current.map(item => item.id === detail.queueId ? { ...item, attemptsAllowed: max, attemptsUsed: nextUsed } : item)
          : current.filter(item => item.id !== detail.queueId);
        localStorage.setItem(QUEUE_KEY, JSON.stringify(next));
        window.dispatchEvent(new CustomEvent("orbion-wheel-queue-updated"));
        return next;
      });
      ...
      setHistory(current => { const next = [approved, ...current].slice(0, 20); localStorage.setItem(WHEEL_HISTORY_KEY, JSON.stringify(next)); return next; });
      window.setTimeout(() => flash(remaining > 0 ? `${detail.personName}: prêmio aprovado. Restam ${remaining} tentativa${remaining === 1 ? "" : "s"}.` : `${detail.personName}: prêmio aprovado e participação concluída.`), 20);
    };
    sync(); loadPeople();
  ...
  const attemptsForNew = attemptsUnlocked ? Math.max(1, Math.min(20, attempts)) : 1;
  const addExisting = () => {
    const person = people.find(item => item.id === personId); if (!person) return;
    persist([...queue, { id: `queue-${Date.now()}`, personId: person.id, personName: person.name, type, createdAt: new Date().toISOString(), source: "earned", attemptsAllowed: attemptsForNew, attemptsUsed: 0 }]);
    flash(`${person.name} adicionado com ${attemptsForNew} tentativa${attemptsForNew === 1 ? "" : "s"}.`);
  };
  const addManual = () => {
    if (!manualName.trim()) return flash("Digite o nome da pessoa para adicionar manualmente.");
    persist([...queue, { id: `queue-${Date.now()}`, personName: manualName.trim(), type, createdAt: new Date().toISOString(), source: "manual", attemptsAllowed: attemptsForNew, attemptsUsed: 0 }]);
    setManualName(""); flash(`Pessoa adicionada com ${attemptsForNew} tentativa${attemptsForNew === 1 ? "" : "s"}.`);
  };
  const changeType = (id: string, nextType: QueueType) => persist(queue.map(item => item.id === id ? { ...item, type: nextType } : item));
  const remove = (id: string) => { if (active?.queueId === id) { localStorage.removeItem(ACTIVE_TURN_KEY); setActive(null); } persist(queue.filter(item => item.id !== id)); };
  const release = (entry: QueueEntry) => {
    const next: ActiveTurn = { queueId: entry.id, personId: entry.personId, personName: entry.personName, type: entry.type, startedAt: new Date().toISOString(), attemptsAllowed: allowed(entry), attemptsUsed: used(entry) };
    localStorage.setItem(ACTIVE_TURN_KEY, JSON.stringify(next)); setActive(next);
    window.dispatchEvent(new CustomEvent("orbion-wheel-turn-changed", { detail: next })); selectWheelMode(entry.type);
    flash(`Vez de ${entry.personName}. Tentativa ${used(entry) + 1} de ${allowed(entry)} liberada.`); window.scrollTo({ top: 0, behavior: "smooth" });
  };
  ...
  return <>{createPortal(<section className="wheel-queue-shell">
    <div className="wheel-queue-head"><div><span><ShieldCheck /> CONTROLE DO GESTOR</span><h2>Fila de giros</h2><p>Você decide quem gira, qual roleta será usada, quantas tentativas terá e quando o prêmio será aprovado.</p></div><div className={`wheel-active-badge ${active ? "live" : ""}`}><i />{active ? `Vez de ${active.personName}` : "Nenhum giro liberado"}</div></div>

    <div className="wheel-attempt-control"><div><span>{attemptsUnlocked ? <Unlock /> : <Lock />}</span><div><strong>Tentativas por pessoa</strong><small>Padrão: 1 tentativa. Destrave o cadeado para liberar mais.</small></div></div><button className={attemptsUnlocked ? "unlocked" : ""} onClick={() => { setAttemptsUnlocked(value => !value); if (attemptsUnlocked) setAttempts(1); }}>{attemptsUnlocked ? <Unlock /> : <Lock />}{attemptsUnlocked ? "Mais tentativas liberadas" : "Liberar mais de uma"}</button><input type="number" min="1" max="20" disabled={!attemptsUnlocked} value={attemptsUnlocked ? attempts : 1} onChange={e => setAttempts(Math.max(1, Math.min(20, Number(e.target.value) || 1)))} /></div>
    ...
    {visibleQueue.length === 0 ? <div className="wheel-queue-empty"><Dices /><strong>Fila vazia</strong><span>Adicione alguém acima quando quiser liberar um giro.</span></div> : <div className="wheel-queue-list">{visibleQueue.map((entry, index) => {
      ...
      return <article key={entry.id} className={isActive ? "active" : ""}><div className="queue-position">{String(index + 1).padStart(2, "0")}</div><div className="queue-avatar"><span>{initials(entry.personName)}</span></div><div className="queue-person"><strong>{entry.personName}</strong><span>{entry.source === "manual" ? "Adicionado manualmente" : "Colaborador do time"} · {used(entry)}/{allowed(entry)} giros usados</span></div><select value={entry.type} disabled={isActive} onChange={e => changeType(entry.id, e.target.value as QueueType)}><option value="classic">Roleta Clássica</option><option value="premium">Roleta Premium</option></select><div className="queue-attempt-badge">{Math.max(0, allowed(entry) - used(entry))} restante{Math.max(0, allowed(entry) - used(entry)) === 1 ? "" : "s"}</div><button className={`release ${isActive ? "released" : ""}`} disabled={!!active && !isActive} onClick={() => !isActive && release(entry)}>{isActive ? <><Zap />Liberado</> : <><Play />Liberar giro</>}</button><button className="remove" onClick={() => remove(entry.id)} title="Remover da fila"><Trash2 /></button></article>;
    })}</div>}
...
function selectWheelMode(type: QueueType) { const buttons = Array.from(document.querySelectorAll<HTMLButtonElement>("main button")); const desired = type === "premium" ? "ROLETA PREMIUM" : "ROLETA CLÁSSICA"; buttons.find(item => (item.textContent || "").toUpperCase().includes(desired))?.click(); }
