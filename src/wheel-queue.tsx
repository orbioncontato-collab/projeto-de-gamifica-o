...
export function WheelQueue() {
  ...
  useEffect(() => {
    ...
    const approve = (event: Event) => {
      ...
      if (!detail) return;
      const currentQueue = normalizeQueue(readList<QueueEntry>(QUEUE_KEY));
      const target = currentQueue.find(item => item.id === detail.queueId);
      let remaining = 0;
      if (target) {
        ...
        const next = remaining > 0
          ? currentQueue.map(item => item.id === detail.queueId ? { ...item, attemptsAllowed: max, attemptsUsed: nextUsed } : item)
          : currentQueue.filter(item => item.id !== detail.queueId);
      ...
      const nextHistory = [approved, ...readList<Approval>(WHEEL_HISTORY_KEY)].slice(0, 20);
      localStorage.setItem(WHEEL_HISTORY_KEY, JSON.stringify(nextHistory));
      setHistory(nextHistory);
      window.dispatchEvent(new CustomEvent("orbion-wheel-queue-updated"));
      setToast(remaining > 0 ? `${detail.personName}: prêmio aprovado. Restam ${remaining} tentativa${remaining === 1 ? "" : "s"}.` : `${detail.personName}: prêmio aprovado e participação concluída.`);
      window.setTimeout(() => setToast(null), 2600);
    };

    const handleStorage = (event: StorageEvent) => {
      if ([QUEUE_KEY, ACTIVE_TURN_KEY, WHEEL_HISTORY_KEY, PEOPLE_KEY, OVERRIDES_KEY].includes(event.key || "")) load();
    };
    ...
    return () => {
      observer.disconnect();
      window.removeEventListener("orbion-wheel-prize-approved", approve);
  ...
  const addManual = () => {
    ...
    setManualName("");
    flash(`Pessoa adicionada com ${attemptsForNew} tentativa${attemptsForNew === 1 ? "" : "s"}.`);
  };

  const changeType = (id: string, nextType: QueueType) => persist(queue.map(item => item.id === id ? { ...item, type: nextType } : item));
  const remove = (id: string) => {
    if (active?.queueId === id) {
      localStorage.removeItem(ACTIVE_TURN_KEY);
      setActive(null);
      window.dispatchEvent(new CustomEvent("orbion-wheel-turn-changed"));
    }
  ...
  const release = (entry: QueueEntry) => {
    ...
    flash(`Vez de ${entry.personName}. Tentativa ${used(entry) + 1} de ${allowed(entry)} liberada.`);
  ...
  return <>
    {createPortal(<section className="wheel-queue-shell">
    ...
    {toast && <div className="wheel-queue-toast"><Check />{toast}</div>}
  </>;
}

function WheelType({ value, onChange }: { value: QueueType; onChange: (value: QueueType) => void }) {
  return <select value={value} onChange={e => onChange(e.target.value as QueueType)}><option value="classic">Roleta Clássica</option><option value="premium">Roleta Premium</option></select>;
}
...
function selectWheelMode(type: QueueType) {
  ...
  buttons.find(item => (item.textContent || "").toUpperCase().includes(desired))?.click();
}
